                                    ...
                                    if( $user_ID != $current->post_author  && ($is_invidual || is_super_admin()) ): ?>
                                        <?php if( $current->post_status == 'publish' || $current->post_status == 'unpause'): ?>
                                            ...
                                            if( $mj_ac ): ?>
                                                ...
                                                <?php else: ?>
                                                    <button onclick="window.location.href='<?php echo get_permalink($mj_ac['0']->ID); ?>'"class="btn-submit btn-orders  waves-effect waves-light <?php echo $disableClass; ?>" ><?php echo sprintf(__('ORDER (<span class="mjob-price">%s</span>)', ET_DOMAIN), $current->et_budget_text) ; ?></button>
                                                <?php endif; ?>
                                            <?php else: ?>
